		...
		foreach ( $banned_ips_range_post as $banned_ip_range ) {
			...
			if ( sizeof( $range ) === 2 ) {
				...
				if ( $admin_login === 'admin' && ( check_ip_within_range( ban_get_ip(), $range_start, $range_end ) ) ) {
					$text .= '<p style="color: blue;">' . sprintf( __( 'The Admin\'s IP \'%1$s\' Fall Within This Range (%2$s - %3$s) And Will Not Be Added To Ban List', 'wp-ban' ), esc_html( ban_get_ip() ), esc_html( $range_start ), esc_html( $range_end ) ) . '</p>';
				} else {
